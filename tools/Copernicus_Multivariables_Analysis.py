...
def plot_spatial_map(data, lon, lat, zone, title, output_file, cmap='viridis', contours=None):
    """Mapa espacial optimizado por zona"""
    if zone == 'Caribbean_Z':
        fig = plt.figure(figsize=(14, 10))
        extent = [-85, -60, 9, 25]
        cbar_pos = [0.90, 0.15, 0.02, 0.7]  # Ajustado para Caribbean
    elif zone == 'Europe_Z':
        fig = plt.figure(figsize=(10, 14))
        extent = [-30, 1, 10, 54]
        cbar_pos = [0.92, 0.15, 0.02, 0.7]  # Ajustado para Europe (figura más alta)
    elif zone == 'Gibraltar_Z':
        fig = plt.figure(figsize=(10, 14))
        extent = [-10, 1, 30, 40]
        cbar_pos = [0.92, 0.15, 0.02, 0.7]  # Ajustado para Gibraltar (figura más alta)
    ...
    cbar_ax = fig.add_axes(cbar_pos)  # Usar posición ajustada por zona
...
def plot_vector_map(u, v, lon_grid, lat_grid, zone, title, output_file, density=3, scale=None, cmap='plasma'):
    """Campo vectorial optimizado por zona"""
    if zone == 'Caribbean_Z':
        ...
        extent = [-30, 1, 10, 54]
        cbar_pos = [0.92, 0.15, 0.02, 0.7]  # Ajustado para Europe
    elif zone == 'Gibraltar_Z':
        fig = plt.figure(figsize=(12, 16))
        extent = [-10, 1, 30, 40]
        cbar_pos = [0.92, 0.15, 0.02, 0.7]  # Ajustado para Gibraltar
    ...
    if scale is None:
        scale = 200 if max_magnitude > 10 else 40
    ...
    ax.set_title(title, fontsize=14, fontweight='bold', pad=20, loc='center')
    plt.savefig(output_file, dpi=500, bbox_inches='tight')
    plt.close()

def animate_variable(var, lon, lat, times, zone, output_file, title_base, interval_ms=1000, cmap='tab20b'):
    """Animación escalar optimizada por zona"""
    if zone == 'Caribbean_Z':
...
def animate_vector_field(u, v, lon_grid, lat_grid, times, zone, output_file, title_base, 
                        interval_ms=1000, density=4, scale=None, cmap='plasma'):
    """Animación vectorial optimizada por zona"""
    if zone == 'Caribbean_Z':
...
def plot_wind_time_series(u_wind, v_wind, times, output_file, title, location_info):
    """Gráfico de serie temporal de viento (SIN CAMBIOS)"""
    fig = plt.figure(figsize=(16, 10))
    gs = plt.GridSpec(3, 1, height_ratios=[1, 1, 0.08], hspace=0.3)
...
def main():
    ...
    path = "/media/amilcar/STORE/COMPARTIDA/TRABAJO/01_PYTHON_PROJECTS/DATA_PROCESSING/Making_IntegratedFiles/Outputs_Copernicus/integrated_european_file_20250816_20250819.nc"
    print("Zone Names: 'Europe_Z', 'Gibraltar_Z', 'Caribbean_Z'")
    zone = input('Type the Zone name: ').strip()
    ...
    # 1. Animación de altura de ola
    print("\n1. Generando animación de altura significativa de ola...")
    try:
        wave_height_vars = [v for v in var_names if any(keyword.lower() in v.lower() 
                                                       for keyword in ['VHM0', 'swh', 'hs', 'wave_height', 'significant'])]
        if wave_height_vars:
            wave_height_name = wave_height_vars[0]
            wave_height = get_variable(ds, wave_height_name)
            animate_variable(wave_height, lon_grid, lat_grid, ds.time, zone,
                           "1_significant_wave_height_animation.gif",
                           "Significant Wave Height", cmap='tab20b', interval_ms=500)
            print("✓ Animación de altura de ola guardada")
        else:
            print("✗ No se encontró variable de altura significativa de ola")
    except Exception as e:
        print(f"✗ Error en animación de ola: {e}")
    
    # 2. Campo vectorial del viento
    ...
    try:
        ...
        if u_wind_vars and v_wind_vars:
            ...
            plot_vector_map(u_wind.isel(time=0), v_wind.isel(time=0), lon_grid, lat_grid, zone,
                          f"Wind Vector Field - {timestamp}",
                          "2_wind_vector_field.png", density=8, scale=200, cmap='coolwarm')
            print("✓ Campo vectorial del viento guardado")
        else:
            print("✗ No se encontraron componentes U/V del viento")
    except Exception as e:
        print(f"✗ Error en campo vectorial del viento: {e}")
    
    # 3. Animación de corrientes marinas
    ...
    try:
        ...
        if u_current_vars and v_current_vars:
            ...
            animate_vector_field(u_current.isel(time=slice(0, n_frames)),
                               ...
                               lon_grid, lat_grid, ds.time.isel(time=slice(0, n_frames)), zone,
                               "3_ocean_currents_animation.gif",
                               "Ocean Currents Vector Field",
                               interval_ms=800, density=8, scale=40, cmap='viridis')
            print("✓ Animación de corrientes marinas guardada")
        else:
            print("✗ No se encontraron componentes U/V de corrientes marinas")
    except Exception as e:
        print(f"✗ Error en animación de corrientes: {e}")
    
    # 4. Análisis de viento en punto
    ...
    try:
        if 'u_wind' in locals() and 'v_wind' in locals():
            ...
            print("✓ Análisis completo de viento guardado")
        else:
